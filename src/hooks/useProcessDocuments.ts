...
export interface ProcessDocFolder {
  id: string;
  process_id: string;
...
export function useProcessDocuments(processId: string | undefined) {
  ...
  const key = ["process-documents", processId];

  const foldersQuery = useQuery({
    queryKey: [...key, "folders"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("process_doc_folders" as any)
        .select("*")
        .eq("process_id", processId!)
        .order("sort_order");
      if (error) throw error;
      return data as unknown as ProcessDocFolder[];
    },
    enabled: !!processId,
  });
  ...
  const deleteFolder = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("process_doc_folders" as any).delete().eq("id", id);
      if (error) throw error;
    },
  ...
  const toggleFolderPublic = useMutation({
    mutationFn: async (params: { id: string; isPublic: boolean }) => {
      ...
      const { error } = await supabase.from("process_doc_folders" as any).update(update).eq("id", params.id);
      if (error) throw error;
    },
  ...
  const deleteDocument = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("process_documents" as any).delete().eq("id", id);
      if (error) throw error;
    },
  ...
  const toggleDocPublic = useMutation({
    mutationFn: async (params: { id: string; isPublic: boolean }) => {
      ...
      const { error } = await supabase.from("process_documents" as any).update(update).eq("id", params.id);
      if (error) throw error;
    },
    onSuccess: () => { invalidateAll(); toast.success("Visibilidade atualizada!"); },
    onError: (e: any) => toast.error(e.message),
  ...
  const uploadFile = async (file: File, folderId?: string | null): Promise<ProcessDocument> => {
    const path = `${tenantId}/processes/${processId}/${crypto.randomUUID()}-${file.name}`;
  ...
  const uploadImage = async (file: File): Promise<string> => {
    const path = `${tenantId}/processes/${processId}/images/${crypto.randomUUID()}-${file.name}`;
    ...
    mutationFn: async (params: { isPublic: boolean }) => {
      ...
      for (const folder of allFolders) {
        if (folder.is_public !== params.isPublic) {
          ...
          const { error } = await supabase.from("process_doc_folders" as any).update(update).eq("id", folder.id);
          if (error) throw error;
        }
      }
      for (const doc of allDocs) {
        if (doc.is_public !== params.isPublic) {
          const update: any = { is_public: params.isPublic };
          update.public_token = params.isPublic ? generateToken() : null;
          const { error } = await supabase.from("process_documents" as any).update(update).eq("id", doc.id);
          if (error) throw error;
        }
      }
    },
    ...
    mutationFn: async (params: { folderId: string; isPublic: boolean }) => {
      ...
      for (const fId of folderIds) {
        ...
        if (folder && folder.is_public !== params.isPublic) {
          const update: any = { is_public: params.isPublic };
          update.public_token = params.isPublic ? generateToken() : null;
          const { error } = await supabase.from("process_doc_folders" as any).update(update).eq("id", fId);
          if (error) throw error;
        }
      }
